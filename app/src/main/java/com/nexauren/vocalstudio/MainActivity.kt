...
class MainActivity : AppCompatActivity() {
        ...
        web.webChromeClient = object : WebChromeClient() {
            override fun onPermissionRequest(request: PermissionRequest) {
                runOnUiThread {
                    ...
                    val audioOnly = request.resources.all {
                        it == PermissionRequest.RESOURCE_AUDIO_CAPTURE
                    }
                    if (!audioOnly || !origin.startsWith("file:")) {
                        request.deny()
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == micRequest) {
            ...
            web.evaluateJavascript(
                "window.dispatchEvent(new Event('androidPermissionChanged'))",
                null
            )
        }
    }
    ...
    private fun saveExport() {
        try {
            ...
            Toast.makeText(
                this,
                "Exported to Music/Nexauren Vocal Studio",
                Toast.LENGTH_LONG
            ).show()
        } catch (e: Exception) {
            Toast.makeText(
    ...
    inner class AppBridge {
        ...
        @JavascriptInterface
        fun requestStorage() = runOnUiThread {
        ...
        fun startExport(name: String) {
            exportName = name
            exportParts.setLength(0)
        }

        @JavascriptInterface
        fun appendExportChunk(chunk: String) {
            exportParts.append(chunk)
        }

        @JavascriptInterface
        fun finishExport() = runOnUiThread {
            saveExport()
        }
    }
